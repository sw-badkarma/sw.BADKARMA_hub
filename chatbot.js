// ==========================================
// LOGICA CHATBOT-ULUI KARMA AI (Ediția cu Diacritice, Online & Typewriter)
// ==========================================
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupOnlineIndicator() {
        ...
        statusContainer.style.cssText = 'display: flex; align-items: center; justify-content: space-between; padding: 4px 0 8px 4px; font-size: 0.75rem; color: #fff; font-weight: bold;';
        ...
        helpBtn.style.cssText = 'background: rgba(255, 255, 255, 0.15); border: none; color: #00ff66; padding: 2px 6px; border-radius: 3px; cursor: pointer; font-size: 0.7rem; font-weight: bold; font-family: monospace; transition: background 0.2s;';
        
        helpBtn.onmouseover = () => helpBtn.style.background = 'rgba(255, 255, 255, 0.3)';
        helpBtn.onmouseout = () => helpBtn.style.background = 'rgba(255, 255, 255, 0.15)';
        ...
        leftGroup.appendChild(greenDot);
        leftGroup.appendChild(statusText);
        leftGroup.appendChild(helpBtn);

        statusContainer.appendChild(leftGroup);
    ...
    function getBotResponse(input) {
        ...
        if (cleanText === 'help' || cleanText === 'ajutor' || cleanText === 'comenzi') {
            return "<strong>🤖 Meniu Ajutor KARMA AI:</strong><br>" +
                   "Iată ce mă poți întreba:<br>" +
                   "• <strong>Loadout / Echipare</strong> (pentru arme în BR sau MP)<br>" +
                   "• <strong>Lucky Draw</strong> (calculator prețuri CP per regiune)<br>" +
                   "• <strong>Meta, Arme, Perks</strong> (sfaturi de echipament)<br>" +
                   "• <strong>Setări BR / Setări MP</strong> (configurări pro)<br>" +
                   "• <strong>Anti-lag, FPS</strong> (optimizare joc)<br>" +
                   "• <strong>Degetare / Mănuși</strong> (ghid accesorii tactile)<br>" +
                   "• <strong>Atașamente</strong> (Muzzle, Barrel, Laser, Stock etc.)";
        }
        ...
        const responses = {
            'salut,salutare,buna,sal,neata,buna seara,buna ziua': "<strong>Salut</strong>, cu ce te pot ajuta?",
            'tutorial, gaming': "Vrei un tutorial pentru alegerea manșoanelor tactile (degetarelor) sau unul pentru întreținerea acestora?",
            ...
            'cum te cheama, cum te numesti, ce nume ai, care este numele tau, ai cumva un nume,imi poti spune numele tau,nume, numele': "<strong>KARMA AI</strong> este numele meu. Cu ce te pot ajuta?",
            ...
            'dezvoltator, creat, creatorul tau, cine te-a creat': "<strong>Nenicioiu Cristian</strong>, el este creatorul si dezvoltatorul meu si al acestei aplicații.",
            ...
            'tutorial,alegere, alegerea, degetare,mansoane,finger sleeves,alegere degetare,tactile,gaming': "<strong>Cum să îți alegi Manșoanele Tactile (Degetare):</strong><br>" +
            ...
            'tutorial,intretinere,intretinerea,spalare degetare,curatare mansoane,pastrare,intretinere degetare,intretinere mansoane': "<strong>Întreținerea Manșoanelor de Gaming:</strong><br>" +
            ...
            'atasament,atasamente,gunsmith,ghid atasamente,ghid complet atasamente': "<strong>Ghidul Complet al Atașamentelor în Gunsmith:</strong><br>" +
        ...
        let celMaiBunRaspuns = "Momentan nu te pot ajuta cu această informație... Întreabă-mă despre loadout-uri, setări pro, degetare, întreținere sau atașamente!";
        let scorMaxim = 0;
    ...
    // Funcție nouă pentru efectul de scriere (Typewriter) compatibil cu HTML
    function addTypewriterMessage(sender, htmlMessage, speed = 12) {
        if (!chatWindow) return;
        const p = document.createElement('p');
        ...
        // Împarte mesajul în tokeni (taguri HTML vs bucăți de text)
        const tokens = htmlMessage.match(/<[^>]*>|[^<]+/g) || [];
        let tokenIndex = 0;
        let charIndex = 0;
        ...
        function type() {
            if (tokenIndex < tokens.length) {
                ...
                if (currentToken.startsWith('<')) {
                    // Dacă e tag HTML, îl afișează instant
                    contentSpan.innerHTML += currentToken;
                    tokenIndex++;
                    setTimeout(type, 0);
                } else {
                    // Dacă e text, tastează literă cu literă
                    if (charIndex < currentToken.length) {
                        contentSpan.innerHTML += currentToken.charAt(charIndex);
                        charIndex++;
                        chatWindow.scrollTop = chatWindow.scrollHeight;
    ...
    if (sendBtn && userInput) {
        sendBtn.addEventListener('click', () => {
            ...
            setTimeout(() => {
                ...
                // Dacă răspunsul este mai lung de 60 de caractere, folosim efectul de tastare. 
                // Altfel, îl afișăm instantaneu.
                if (response.length > 60) {
                    addTypewriterMessage('KARMA AI', response, 10); // 10ms per caracter (modifică pentru viteză)
                } else {
